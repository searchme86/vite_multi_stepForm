...
import { Input, Textarea, Chip, Button, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem } from "@heroui/react";
...
const BlogContentStep: React.FC = () => {
  ...
  const handleFormatChange = (key: string) => {
    setSelectedFormat(formatOptions.find(option => option.key === key)?.label || "Normal");
    ...
    if (selectedText) {
      const newContent = contentValue.substring(0, start) + formattedText + contentValue.substring(end);
  ...
  return (
    <div className="space-y-6">
      <div className="bg-default-50 p-4 rounded-lg mb-6">
        <h3 className="text-lg font-medium mb-2">블로그 컨텐츠 입력 안내</h3>
        <p className="text-default-600">
          블로그의 본문 내용과 관련 태그를 입력해주세요. 태그는 검색과 분류에 도움이 됩니다.
          마크다운 형식으로 내용을 작성하면 미리보기에서 서식이 적용된 상태로 확인할 수 있습니다.
          태그는 최대 5개까지 입력 가능하며, 내용은 최소 5자 이상 작성해주세요.
        </p>
      </div>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {tags.length === 0 && (
              <p className="text-default-500 text-sm">추가된 태그가 없습니다.</p>
            )}
            {tags.length >= 5 && (
              <p className="text-warning text-xs w-full mt-1">최대 5개의 태그만 추가할 수 있습니다.</p>
            )}
          </div>
      ...
      >
        <div className="space-y-2">
          ...
          <div className="flex justify-between items-center">
            <span className={`text-xs ${characterCount < 5 ? "text-danger" : "text-default-500"}`}>
