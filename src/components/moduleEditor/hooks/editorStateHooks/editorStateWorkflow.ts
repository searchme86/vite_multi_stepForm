// 📁 hooks/useEditorState/editorStateWorkflow.ts
...
// ✨ [워크플로우 함수들] useCallback 제거하여 Hook 규칙 위반 해결
...
const completeStructureSetup = (
  ...
  return (validSectionInputCollection: string[]) => {
    console.log(
      '🎛️ [ACTION] completeStructureSetup 호출:',
    ...
    try {
      ...
      console.error('❌ [ACTION] 구조 설정 완료 실패:', error);
      if (showToastFunction) {
        showToastFunction({
          title: '구조 설정 실패',
...
const navigateToStructureStep = (
  ...
  // ✅ useCallback 제거하고 일반 함수 반환
  return () => {
    console.log('🎛️ [ACTION] navigateToStructureStep 호출');
    ...
    try {
      ...
      console.error('❌ [ACTION] 구조 단계 이동 실패:', error);
    }
  };
};
...
const setActiveEditor = (
  ...
  // ✅ useCallback 제거하고 일반 함수 반환
  return (specificParagraphIdToActivate: string) => {
    console.log(
      '🎛️ [ACTION] setActiveEditor 호출:',
    ...
    try {
      ...
      setTimeout(() => {
        try {
          ...
          console.error('❌ [ACTION] 스크롤 실패:', scrollError);
          // 1. 스크롤 실패해도 에디터 활성화 자체에는 문제없도록 에러 격리
          // 2. 스크롤은 UX 개선 기능이므로 실패해도 핵심 기능에 영향 없음
        }
      }, 200);
    } catch (error) {
      console.error('❌ [ACTION] 에디터 활성화 실패:', error);
    }
  };
};
...
const switchPreviewMode = (
  ...
  // ✅ useCallback 제거하고 일반 함수 반환
  return () => {
    console.log('🎛️ [ACTION] switchPreviewMode 호출');
    ...
    try {
      ...
      console.error('❌ [ACTION] 미리보기 모드 전환 실패:', error);
    }
  };
};
